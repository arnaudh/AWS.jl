...
- `"KmsKeyId"`: The Amazon Resource Name (ARN) of a Amazon Web Services Key Management
  ...
  Store Volumes.
- `"ShadowProductionVariants"`:  Array of ProductionVariant objects. There is one for each
  model that you want to host at this endpoint in shadow mode with production traffic
  replicated from the model specified on ProductionVariants.If you use this field, you can
...
this API to schedule an experiment to compare model variants on a Amazon SageMaker
inference endpoint. For more information about inference experiments, see Shadow tests.
Amazon SageMaker begins your experiment at the scheduled time and routes traffic to your
endpoint's model variants based on your specified configuration.   While the experiment is
in progress or after it has concluded, you can view metrics that compare your model
...
- `model_variants`:  Array of ModelVariantConfigSummary objects. There is one for each
...
- `shadow_mode_config`:  Shows which variant is the production variant and which variant is
  the shadow variant. For the shadow variant, also shows the sampling percentage.
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"DataStorageConfig"`:  The storage configuration for the inference experiment. This is
  an optional parameter that you can use for data capture. For more information, see Capture
  data.
...
- `"SortBy"`: The parameter by which to sort the results. The default is CreationTime.
- `"SortOrder"`: The sort order for the results. The default is Ascending.
- `"SpaceNameEquals"`: A parameter to search by space name.
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"DesiredModelVariants"`:  Array of ModelVariantConfig objects. There is one for each
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"DataStorageConfig"`: The Amazon S3 storage configuration for the inference experiment.
- `"Description"`: The description of the inference experiment.
- `"ModelVariants"`:  Array of ModelVariantConfigSummary objects. There is one for each
...
- `"ShadowModeConfig"`: The Amazon S3 storage configuration for the inference experiment.
"""
