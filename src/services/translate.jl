...
volumes of text across multiple documents at once. For batch translation, the input
documents must share the same source language. You can specify one or more target
languages. Batch translation translates each input document into each of the target
languages. For more information, see Asynchronous batch processing  Batch translation jobs
can be described with the DescribeTextTranslationJob operation, listed with the
...
- `source_language_code`: The language code of the input language. For a list of language
  codes, see Supported languages. Amazon Translate does not automatically detect a source
...
- `target_language_codes`: The target languages of the translation job. Enter up to 10
  ...
  is two or five characters long. For a list of language codes, see Supported languages.
